...
import { GravityBoss } from '../entities/offensive/GravityBoss';
import { ConstructBoss } from '../entities/offensive/ConstructBoss';
import { PhantomBoss } from '../entities/offensive/PhantomBoss';
...
import {
  ...
  BOSS4_HEALTH_MULTIPLIER,
  BOSS4_SPAWN_OFFSET_Y,
  BOSS5_HEALTH_MULTIPLIER,
  BOSS5_SPAWN_OFFSET_Y,
  BOSS6_HEALTH_MULTIPLIER,
  BOSS6_SPAWN_OFFSET_Y,
  MULTIBALL_MIN_ANGLE,
  MULTIBALL_MAX_ANGLE,
...
export class Game {
  ...
  private setupBrickDestructionCallbacks(): void {
    ...
    for (const brick of bricks) {
      brick.setOnDestroyCallback((destroyedBrick, info) => {
        ...
        if ((brickType === BrickType.BOSS_1 ||
          brickType === BrickType.BOSS_2 ||
          brickType === BrickType.BOSS_3 ||
          brickType === BrickType.BOSS_4 ||
          brickType === BrickType.BOSS_5 ||
          brickType === BrickType.BOSS_6) && !this.boss) {
  ...
  private activateBoss(brick: Brick, info: { centerX: number; centerY: number }): void {
    ...
    } else if (brickType === BrickType.BOSS_4) {
      bossHealth = baseHealth * BOSS4_HEALTH_MULTIPLIER;
      spawnOffsetY = brick.getHeight() * BOSS4_SPAWN_OFFSET_Y;
    } else if (brickType === BrickType.BOSS_5) {
      bossHealth = baseHealth * BOSS5_HEALTH_MULTIPLIER;
      spawnOffsetY = brick.getHeight() * BOSS5_SPAWN_OFFSET_Y;
    } else if (brickType === BrickType.BOSS_6) {
      bossHealth = baseHealth * BOSS6_HEALTH_MULTIPLIER;
      spawnOffsetY = brick.getHeight() * BOSS6_SPAWN_OFFSET_Y;
    } else {
  ...
  loadLevel(levelConfig: LevelConfig): void {
    ...
    const hasBoss = this.level.getBricks().some(brick =>
      ...
      brick.getType() === BrickType.BOSS_4 ||
      brick.getType() === BrickType.BOSS_5 ||
      brick.getType() === BrickType.BOSS_6
    );
  ...
  private update(deltaTime: number): void {
    ...
    if (this.boss && this.boss.isActive()) {
      // Special update for GravityBoss to handle ball interaction
      if (this.boss instanceof GravityBoss) {
        this.boss.updateWithBall(effectiveDeltaTime, this.bat.getCenterX(), this.bat.getCenterY(), this.getPrimaryBall());
      } else {
          ...
          (x, y) => {
            ...
            if (this.boss) {
              ...
              if (this.boss instanceof Boss1) {
                ...
                bossType = 'BOSS_3';
              } else if (this.boss instanceof GravityBoss) {
                bossType = 'BOSS_4';
              } else if (this.boss instanceof ConstructBoss) {
                bossType = 'BOSS_5';
              } else if (this.boss instanceof PhantomBoss) {
                bossType = 'BOSS_6';
              } else {
                bossType = 'UNKNOWN';
              }
